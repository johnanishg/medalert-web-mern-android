@echo off
REM MedAlert Kiosk Mode Setup Script for Windows
REM This script configures the Android device for kiosk mode
...
REM Enable auto-start (MIUI specific)
...
echo Please manually enable "Autostart" in the settings that opened, then press any key to continue...
pause
echo.

REM Set device owner (requires factory reset - optional)
...
echo.
echo Do you want to try setting Device Owner? (y/n)
set /p setOwner="> "
if /i "%setOwner%"=="y" (
...
REM Start the app in lock task mode
...
REM Additional MIUI optimizations
